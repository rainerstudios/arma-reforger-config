...
const ServerConfigForm = function() {
  const [workshopData, setWorkshopData] = useState(null);
  ...
  const handleModChange = (mods) => {
    setFormData(prevData => {
      ...
      // If mods are added, remove PS5 platform
      if (mods.length > 0) {
      ...
      <form className="space-y-6">
        ...
        <FormSection title="Game Configuration">
          <div className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">Mods</label>
              <select
                multiple
                name="game.mods"
                className="mt-1 block w-full rounded-md border border-gray-300 py-2 shadow-sm focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500"
